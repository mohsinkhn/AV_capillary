...
def read_reorders(pid_dict, mode='val', thresh=0.05):
    if mode == 'val':
        reorders = pd.read_csv("outputs/val_df.csv")
    else:
        reorders = pd.read_csv("outputs/test_df.csv")

    reorders['pid'] = reorders['productid'].map(pid_dict).astype(int)
    reorders = reorders.loc[reorders.preds > thresh].groupby("UserId")['pid'].apply(list)
    return reorders


def merge_reorders(x):
    r = x['reorders']
    ps = x['pid_list']
    if isinstance(r, list):
        ps = [int(p) for p in ps if p not in r]
        return (r + ps)[:10]
    else:
        return ps
...
if __name__ == "__main__":
    ...
    reorders = read_reorders(pid_dict, MODE, thresh=0.1)
    if MODE == "val":
        results["UserId"] = results.uid.map(uid_rev_dict).astype(int)
        results["reorders"] = results.UserId.map(reorders)
        results["pid_corr"] = results.apply(lambda x: merge_reorders(x), axis=1)
